...
export class JwtMiddleware {
  ...
  validReqGavUser(req: any, res: express.Response, next: express.NextFunction) {
    const token = req.get('Authorization');
    const authToken = token?.split(' ')[1];
  
    jwt.verify(authToken, `${process.env.JWT_SECRET_GAV}`, { algorithms: ['RS512'] }, (err: any, decoded: any) => {
      if (err) {
        if (err.name === 'JsonWebTokenError') {
          return res.status(401).json({
            error: {
